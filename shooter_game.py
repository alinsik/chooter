...
pygame.display.set_caption("Гра Шутер. Автор: Білоган Аліна")
...
class Player(GameSprite):
    ...
    def fire(self):
        new_bullet = Bullet("bullet.png", self.rect.centerx, self.rect.top,8,(20,40))
...
player = Player("rocket.png", width/2, height-70, 8, (60,70))
...
while not game_over:
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            if e.key == pygame.K_SPACE:
                player.fire()
                shorts -= 1
                if shorts <= 0 and not reload_time:
                    last_shot_time = time()
                    reload_time = True
    ...
    if not finish:
        ...
        if reload_time:
            cur_time = time()
            if cur_time - last_shot_time > 3:
                short = 5
                reload_time = False
            else:
                font3 = pygame.font.Font(None, 20)
                text_reload = font3.render("Зачекай! Йде перезарядка", True, (25,0,0))
